...
const Admin = () => {
  ...
  const clearVideoFile = () => { setVideoFile(null); setVideoFileName(""); if (videoInputRef.current) videoInputRef.current.value = ""; };

  const resetForm = () => {
    setTitulo(""); setDescripcion(""); setCategoria("");
    setVideoUrl(""); setVideoMode("link"); clearImage(); clearVideoFile();
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      if (imageFile) {
        ...
        const { error: uploadError } = await supabase.storage.from("publicaciones").upload(filePath, imageFile);
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from("publicaciones").getPublicUrl(filePath);
      ...
      if (videoMode === "link" && videoUrl.trim()) {
        ...
        const { error: uploadError } = await supabase.storage.from("publicaciones").upload(filePath, videoFile);
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from("publicaciones").getPublicUrl(filePath);
      ...
      const { error } = await supabase.from("publicaciones").insert({
        ...
        categoria, imagen_url, video_url, user_id: user.id,
      });
    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="flex items-center gap-3 mb-8 animate-fade-in-up">
        ...
        <div>
          ...
          <div className="h-px w-32 mt-1.5"
          ...
          >
            ...
            <div className="space-y-1.5">
              <label className="text-xs uppercase tracking-widest text-white/40"
                style={{ fontFamily: "'Oswald', sans-serif" }}>Título *</label>
              <input
                style={inputStyle}
                placeholder="Título de la noticia"
                value={titulo}
                onChange={(e) => setTitulo(e.target.value)}
                onFocus={e => (e.target as HTMLInputElement).style.borderColor = "#CC1E1E"}
            ...
            <div className="space-y-1.5">
              <label className="text-xs uppercase tracking-widest text-white/40"
                style={{ fontFamily: "'Oswald', sans-serif" }}>Texto</label>
              <Textarea
            ...
            <div className="space-y-1.5">
              <label className="text-xs uppercase tracking-widest text-white/40"
                style={{ fontFamily: "'Oswald', sans-serif" }}>Categoría *</label>
              <select
                value={categoria}
                onChange={(e) => setCategoria(e.target.value)}
                style={{ ...inputStyle, cursor: "pointer" }}
                onFocus={e => (e.target as HTMLSelectElement).style.borderColor = "#CC1E1E"}
                onBlur={e => (e.target as HTMLSelectElement).style.borderColor = "rgba(255,255,255,0.1)"}
              >
                <option value="" style={{ background: "#111" }}>Seleccioná un deporte</option>
                {categorias.map((cat) => (
            ...
            <div className="space-y-1.5">
              <label className="text-xs uppercase tracking-widest text-white/40"
                style={{ fontFamily: "'Oswald', sans-serif" }}>Foto</label>
              {imagePreview ? (
                ...
                <button
                  ...
                  onMouseEnter={e => { (e.target as HTMLElement).closest('button')!.style.borderColor = "#CC1E1E"; (e.target as HTMLElement).closest('button')!.style.color = "#CC1E1E"; }}
                  onMouseLeave={e => { (e.target as HTMLElement).closest('button')!.style.borderColor = "rgba(255,255,255,0.1)"; (e.target as HTMLElement).closest('button')!.style.color = "rgba(255,255,255,0.3)"; }}
                >
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest text-white/40"
                style={{ fontFamily: "'Oswald', sans-serif" }}>Video</label>
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    key={mode}
                    onClick={() => { setVideoMode(mode); if (mode === "link") clearVideoFile(); else setVideoUrl(""); }}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold uppercase tracking-widest transition-all"
                    style={{
                      fontFamily: "'Oswald', sans-serif",
                      borderRadius: "2px",
                      background: videoMode === mode ? "#CC1E1E" : "rgba(255,255,255,0.05)",
                      color: videoMode === mode ? "white" : "rgba(255,255,255,0.4)",
                      border: "1px solid",
                      borderColor: videoMode === mode ? "#CC1E1E" : "rgba(255,255,255,0.08)",
                    }}
                  >
                    <Icon className="h-3.5 w-3.5" />
                    {label}
              ...
              {videoMode === "link" ? (
                ...
                <>
                  {videoFileName ? (
                    <div className="flex items-center gap-2 px-3 py-2 text-sm rounded-sm"
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                      <Video className="h-4 w-4 text-white/30" />
                      <span className="truncate flex-1 text-white/60">{videoFileName}</span>
                      <button onClick={clearVideoFile}><X className="h-4 w-4 text-white/30 hover:text-red-500 transition-colors" /></button>
                    </div>
                  ) : (
                    <button
